...
export default function HomePage() {
  ...
  // Reset scrollLeft to 0 when the carousel scrolls out of view
  useEffect(() => {
    const handler = () => {
  ...
  // Section component: header + arrows + carousel
  const Section = ({ title, items, containerRef, theme, viewMoreHref, className = '' }) => {
    ...
    useEffect(() => {
      ...
      const onTouchMove = e => {
        ...
        el.scrollLeft = startScroll + delta * F
        e.preventDefault()
      }
