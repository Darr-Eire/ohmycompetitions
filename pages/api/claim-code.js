...
export default async function handler(req, res) {
  ...
  if (!active) return res.status(400).json({ success: false, message: 'Not eligible or already claimed' })

  const now = new Date()
  if (now > new Date(active.claimExpiresAt)) {
    return res.status(400).json({ success: false, message: 'Claim window expired ⌛' })
  }
