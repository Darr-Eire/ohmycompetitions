...
export default function PartnersPage() {
  return (
    <main className="pages-partners app-background relative min-h-screen p-4 text-white font-orbitron overflow-hidden">
      {/* Futuristic background: soft blobs + starfield + grid dots */}
      <BackgroundFX />

      <div className="relative max-w-5xl mx-auto space-y-10">
        {/* Header */}
        <header className="text-center">
          <div className="competition-top-banner title-gradient mb-4">
            Partners & Sponsors
          </div>
        ...
        >
          ...
          {/* Become Partner CTA (animated shimmer) */}
          <TiltCard>
            <div className="flex flex-col items-center text-center">
        ...
        {/* X Pages That Helped Us Grow (marquee-style) */}
        <GlassCard>
          <SectionTitle>X Pages That Helped Us Grow</SectionTitle>

          {/* Marquee */}
          <Marquee>
            <span>Crypto Pioneer</span>
            <span>Pi Builders Hub</span>
            <span>Pi Pulse Daily</span>
            <span>OMC Updates</span>
            <span>Pioneers Global</span>
          </Marquee>

          <ul className="space-y-3 text-white/90 text-sm text-center mt-4">
...
function GlassCard({ children, className = '' }) {
  return (
    <motion.div
      className={`competition-card flex flex-col bg-white/[0.05] border border-cyan-500/20 backdrop-blur-md rounded-2xl p-6 shadow-[0_0_40px_rgba(34,211,238,0.15)] ${className}`}
...
/* Tilt + hover animated card wrapper */
        ...
        {Array(2).fill(0).map((_, i) => (
          <span key={i} className="mr-10 space-x-10">
            {Array.isArray(children)
              ? children.map((c, idx) => <span key={`${i}-${idx}`}>{c}</span>)
              : children}
          </span>
        ))}
      </div>
