...
import { useFunnelStage } from '../../hooks/useFunnelStages';
 ...
 * API shape expected at /api/funnel/stages:
 *   { success: true, prizePoolPi: number, stages: [{ stage, slug, title, pricePi, startsAt, endsAt, totalTickets, ticketsSold, winners, imageUrl, rooms: [...] }]}
 * If API not present, we auto-mock.
 * ---------------------- */
function useFunnelStages() {
  ...
  // Fallback mock (preserves your UX)
  ...
  // map lists like your old code expected
    ...
    >
      ...
      >
        ...
        <div className="mt-3 text-right">
          <button
            onClick={onClose}
            className="rounded-md bg-cyan-400 text-black px-4 py-2 font-bold"
          >
            Got it
          </button>
        </div>
...
export default function FunnelIndexPage() {
  ...
  const handleJoin = useCallback(
    async ({ slug, stage }) => {
      ...
      try {
        if (stage === 1) {
          const assignedSlug = slug ?? null; // server will place user if null
  ...
  // hero countdown target: earliest nextStart across live stages
