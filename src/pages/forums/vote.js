...
import { FaPoll, FaThumbsUp, FaPlus, FaEye, FaReply, FaClock } from 'react-icons/fa'

export default function VotePage() {
  const [threads, setThreads] = useState([])
  const [loading, setLoading] = useState(true)
  const [showCreateForm, setShowCreateForm] = useState(false)
  const [newVote, setNewVote] = useState({ title: '', body: '' })
  ...
  const fetchThreads = async () => {
    try {
      const res = await fetch('/api/forums/threads?category=vote')
  ...
  const handleCreateVote = async (e) => {
    e.preventDefault()
    if (!newVote.title.trim() || !newVote.body.trim()) {
    ...
    try {
      const res = await fetch('/api/submit/vote', {
        ...
        body: JSON.stringify({
          title: newVote.title,
          content: newVote.body
        })
      })

      if (res.ok) {
        alert('✅ Vote proposal submitted!')
        setNewVote({ title: '', body: '' })
      ...
      alert('❌ Failed to submit vote proposal')
    }
    setCreating(false)
  }
    ...
    <main className="min-h-screen px-4 py-10 bg-gradient-to-r from-[#0f172a] via-[#1e293b] to-[#0f172a] text-white font-orbitron">
      <div className="max-w-4xl mx-auto border border-cyan-700 rounded-2xl p-6 sm:p-8 backdrop-blur-md shadow-[0_0_30px_#00fff055]">
        ...
        <div className="text-center mb-6">
          ...
          >
            <FaPlus />
            {showCreateForm ? 'Cancel' : 'Propose New Vote'}
          </button>
        </div>
        ...
        {showCreateForm && (
          <div className="border border-cyan-600 rounded-xl bg-[#0f172a]/60 p-6 mb-6">
            <h2 className="text-lg font-bold text-cyan-300 mb-4">Propose a Vote</h2>
            <form onSubmit={handleCreateVote} className="space-y-4">
              ...
              <textarea
                value={newVote.body}
                onChange={(e) => setNewVote({...newVote, body: e.target.value})}
                placeholder="Describe the voting options and details..."
              ...
              <div className="flex gap-2">
                <button
                  type="submit"
                  disabled={creating}
                  className="bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white font-bold px-6 py-2 rounded-full transition disabled:opacity-50"
                >
        ...
        <div className="space-y-4">
          {loading ? (
            <div className="text-center py-8">
              <p className="text-cyan-300">Loading votes...</p>
            </div>
          ) : threads.length === 0 ? (
            <div className="text-center py-8">
              <FaPoll className="text-4xl text-gray-400 mx-auto mb-4" />
              <h3 className="text-lg font-bold text-gray-300 mb-2">No votes yet</h3>
              <p className="text-gray-400">Be the first to propose a vote!</p>
            </div>
          ) : (
            threads.map((thread) => (
