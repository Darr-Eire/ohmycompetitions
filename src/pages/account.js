...
const buildReferralLink = (username) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        if (!user) return;
        const rsp = await fetch(`/api/referrals/me?userId=${encodeURIComponent(user.uid || user.piUserId || '')}`);
        if (rsp?.ok) {
  ...
  return (
    <div className="bg-[#0a1024] min-h-[100dvh] max-w-md mx-auto text-white">
      ...
      {/* Tabs */}
      <nav className="px-4 pb-3 pt-2 flex flex-wrap gap-2" role="tablist" aria-label="Account sections">
        {[
          ['tickets', 'Tickets'],
          ['stages', 'Stages'],
          ['rewards', 'Rewards'],
