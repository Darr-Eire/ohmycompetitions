...
import TicketSkillQuestion from './TicketSkillQuestion';
        ...
        >
          {items.map((t, i) => {
            ...
            return (
              <div
                key={`${group.key}-${i}`}
                className="absolute top-0 snap-start transition-transform duration-300 cursor-pointer will-change-transform"
                style={{ left: `${i * OFFSET}px`, zIndex: z, transform: 'translateZ(0)' }}
                onClick={() => onSelect(i)}
