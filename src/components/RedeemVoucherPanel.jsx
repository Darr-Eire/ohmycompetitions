...
export default function RedeemVoucherPanel({ onRedeemed }) {
  ...
  async function submit(e) {
    ...
    if (!valid) {
      setErr('Enter a full code like OMC-ABCD-1234-EFGH');
      inputRef.current?.focus();
      return;
    }
    ...
    try {
      ...
      setMsg(
        `Redeemed ${json.qty} ticket${json.qty > 1 ? 's' : ''} for “${json.competitionSlug}”${
      ...
      setCode('');
      onRedeemed?.(json);
      if (liveRef.current) liveRef.current.textContent = 'Voucher redeemed successfully';
    } catch (e) {
      const message =
        e.name === 'AbortError' ? 'Network timeout. Please try again.' :
        /expired/i.test(e.message) ? 'This code has expired.' :
        /not found|invalid/i.test(e.message) ? 'That code is invalid.' :
        /limit|already/i.test(e.message) ? 'Redemption limit reached for this code.' :
  ...
  async function pasteFromClipboard() {
    try {
      ...
      setErr('Clipboard read blocked by browser. Paste manually (Ctrl/Cmd+V).');
    }
  }
    ...
    <section className="rounded-2xl border border-cyan-400/40 bg-[#0b1220]/95 backdrop-blur-xl p-4 sm:p-5 shadow-[0_0_20px_rgba(34,211,238,0.08)]">
      ...
      <form onSubmit={submit} className="space-y-3" aria-describedby="redeem-help">
        <div className="grid sm:grid-cols-[1fr_auto_auto] gap-2">
          <div className="relative">
            <input
              ...
              placeholder="OMC-XXXX-XXXX-XXXX"
              value={code}
            ...
            <div className="absolute inset-y-0 right-2 flex items-center">
              <span className={`text-[10px] px-2 py-1 rounded-md ${
                ...
                {valid ? (busy ? 'Sending…' : 'Ready') : `${charsLeft} left`}
              </span>
            </div>
          </div>
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            <label className="inline-flex items-center gap-2 cursor-pointer select-none">
              ...
              <span className="text-white/90 text-sm">Reserve now</span>
            </label>
            ...
            <label className="inline-flex items-center gap-2 cursor-pointer select-none">
              ...
              <span className="text-white/90 text-sm">Auto submit</span>
            </label>
          </div>

          <p className="text-[11px] text-white/60">
            Reserving deducts inventory immediately and may assign ticket numbers on success.
          </p>
        </div>
        ...
        <p id="redeem-help" className="text-[11px] text-white/50">
          Format: <code className="px-1 py-0.5 bg-white/10 rounded">OMC-ABCD-1234-EFGH</code>. Press <kbd>Enter</kbd> to submit, <kbd>Esc</kbd> to clear.
        </p>
      </form>
