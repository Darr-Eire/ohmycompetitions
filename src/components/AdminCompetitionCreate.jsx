...
export default function CreateCompetitionPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Send BOTH shapes for compatibility with your API
      const payload = {
        // top-level (old shape)
        slug: form.slug,
        entryFee: toNum(form.entryFee),
        totalTickets: toNum(form.totalTickets, 1),
        title: form.title,
        prize: form.prize,
        theme: form.theme,
        startsAt: form.startsAt,
        endsAt: form.endsAt,
        // legacy name some routes used:
        numberOfWinners: toNum(form.winnersCount, 1),
        // new fields flat (if API reads flat)
        maxPerUser: toNum(form.maxPerUser, 1),
        winnersCount: toNum(form.winnersCount, 1),
        ...
        comp: {
          ...
          piAmount: form.theme === 'free' ? 0 : toNum(form.entryFee),
          status: 'active',
          maxPerUser: toNum(form.maxPerUser, 1),
          winnersCount: toNum(form.winnersCount, 1),
        },
