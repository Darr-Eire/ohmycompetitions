...
/**
 * Layout component
 * @param {{ children: React.ReactNode, fullWidth?: boolean, showMarquee?: boolean }} props
 */
export default function Layout({ children, fullWidth = false, showMarquee = true }) {
  ...
  return (
    <div className="layout font-orbitron text-white">
      ...
      {/* Global Marquee (toggle with showMarquee) */}
      {showMarquee && (
      ...
      {/* Main content area */}
      <main
        className={`content pt-6 pb-8 px-2 min-h-screen space-y-16 ${fullWidth ? '' : 'mx-auto max-w-7xl'}`}
        style={{
