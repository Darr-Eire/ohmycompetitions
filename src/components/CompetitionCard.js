...
export default function CompetitionCard({
  ...
  const entryFeeDisplay = entryFeeNum > 0 ? `${entryFeeNum} π` : t('free_entry', 'Free Entry');

  const imgSrcRaw = imageUrlProp ?? c.imageUrl ?? '/pi.jpeg';
  const img = useMemo(() => {
    const src = String(imgSrcRaw).replace(/\\/g, '/').replace(/^(?!\/|https?:\/\/)/, '/');
    const external = /^https?:\/\//i.test(src);
    return { src, external };
  }, [imgSrcRaw]);

  const total = toNum(c.totalTickets ?? c.ticketsTotal ?? c.capacity, 100);
  const sold = toNum(c.ticketsSold ?? c.entries ?? c.sold, 0);
  ...
  const endAt = endsAtProp || c.endsAt || c.endAt || new Date(Date.now() + 864e5).toISOString();
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      if (s && now < s) {
        setStatus('UPCOMING'); setShowCountdown(false); setTimeLeft(''); return;
      }
      if (!e || now >= e) {
        setStatus('ENDED'); setShowCountdown(false); setTimeLeft(''); return;
      }
      setStatus('LIVE NOW');
      const diff = e - now;
      ...
      setTimeLeft(d > 0 ? `${d}D ${h}H ${m}M` : h > 0 ? `${h}H ${m}M` : `${m}M ${String(sLeft).padStart(2, '0')}S`);
    }, 1000);
  ...
  return (
    <div className={root}>
      ...
      <div className="relative z-10 px-4 py-2 bg-gradient-to-r from-[#00ffd5] via-[#00ccff] to-[#0077ff]">
        <h3 className="text-base font-bold uppercase text-black text-center truncate">{headerTitle}</h3>
      </div>
      ...
      <div className="p-3 text-sm space-y-2">
        ...
        <Row label={t('entry_fee', 'Entry Fee')} value={entryFeeDisplay} />
        <Row
