...
export default function PiCompetitionCard({
  ...
  useEffect(() => {
    const updateTimer = () => {
      ...
      if (now < start) {
        setStatus(comp?.comingSoon ? 'COMING SOON' : 'UPCOMING')
        setTimeLeft('')
  ...
  const statusLabel = willPreSale ? 'PRE-SALE' : status
      ...
      <div className="relative w-full max-w-sm mx-auto p-4 bg-[#0f172a] rounded-xl text-white font-orbitron shadow-xl border-2 border-cyan-400 overflow-hidden">
        ...
        <div className="bg-[#1a1c2e] p-3 rounded-lg border border-cyan-300 text-sm mb-3">
          ...
          <>
            {countryList.length === 0 ? (
              <div className="text-center text-cyan-300/70">No data yet</div>
            ) : (
        ...
        <div className="text-sm space-y-2">
          <p className="flex justify-between">
            <span className="text-cyan-300">Prize:</span>
            <span>{prize}</span>
          </p>
          ...
          <p className="flex justify-between">
            <span className="text-cyan-300">Multiple Winners:</span>
            <span>
              {comp?.comp?.winners
                ? comp.comp.winners
                : comp?.winners
          ...
          <p className="flex justify-between">
            <span className="text-cyan-300">
              {statusLabel === 'PRE-SALE' ? 'Pre-Tickets:' : 'Tickets:'}
            </span>
            <span>
            ...
            >
              ...
              >
                {isSoldOut
                  ...
                  : statusLabel === 'PRE-SALE'
                  ? 'Pre-Book Ticket'
                  : status === 'UPCOMING' || status === 'COMING SOON'
                  ? 'Coming Soon'
                  : 'Enter Now'}
              </button>
            </Link>
